...
// 注册显示隐藏快捷键
const registertGlobalKeymap = (win: BrowserWindow, keymapType: KeymapType, keymap: string): boolean => {
    // 解除绑定
...
const registerKeymapIpc = (win: BrowserWindow) => {
    ipcMain.handle(IPC_MSG_REGISTER_KEYMAP, (_event: IpcMainInvokeEvent, keymapType: KeymapType, keymap: string) => {
        // 重新绑定
